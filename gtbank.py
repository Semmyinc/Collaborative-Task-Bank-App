...
# move.execute("CREATE TABLE gtbank (ctm_id INT(4) PRIMARY KEY AUTO_INCREMENT NOT NULL, Firstname VARCHAR(20) NOT NULL, Surname VARCHAR(20) NOT NULL, Username VARCHAR(20) NOT NULL, Phone VARCHAR(11) UNIQUE NOT NULL, Bvn VARCHAR(11) NOT NULL UNIQUE, Airtime VARCHAR(11) NOT NULL, Data VARCHAR(11) NOT NULL, Email VARCHAR (30) UNIQUE NOT NULL, Password VARCHAR(20) NOT NULL, Confirm_Password VARCHAR(20) NOT NULL, Pin VARCHAR(4) NOT NULL, Confirm_Pin VARCHAR(4) NOT NULL, Balance VARCHAR (20) NOT NULL, Account_Number VARCHAR(10) UNIQUE NOT NULL)")
# move.execute("SHOW TABLES")
# for table in move: 
#     print(table)
class Gtbank:
    ...
    def registration(self):
        ...
        if choice == 'Y':
            ...
            if bvnchoice == 1:
                ...
                quit() 
        record = "INSERT INTO Gtbank (Firstname, Surname, Username, Phone, BVN, Airtime, Data, Email, Password, Confirm_Password, Pin, Confirm_Pin, Balance, Account_Number) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        ...
        server.commit()
        print(move.rowcount, "record(s) inserted successfully")
    ...
    def login(self):    
        ...
        while self.logincount <= 3:
            ...
            # masking password
            self.pword = maskpass.askpass(prompt = 'Password: ', mask = '*')
            recordlogin = "SELECT * FROM gtbank WHERE Email = %s AND Password = %s"
    ...
    def menu(self):
        ...
        if choice == 1:
            self.pin = int(maskpass.askpass(prompt = 'Pin: ', mask = '*'))
            # record = "SELECT * FROM Gtbank WHERE Pin = %s"
            # val = (self.pin)
            # move.execute(record,val)
            # schedule = move.fetchone()
            # print(schedule)
        ...
        # Deposit
            ...
            # move.execute(record, val)
            # schedule = move.fetchone()
            gtcobal = float(self.schedule[-2])
            deposit_amount = float(input('Deposit Amount: '))
            self.balance = gtcobal + deposit_amount
            # self.balance = self.uname
            record = "UPDATE gtbank SET  Balance=%s WHERE Pin=%s"
        ...
        # Withdrawal/Transfer
            ...
            if self.pin == self.schedule[-3]:
                ...
                if total_transfer_amount <= self.balance:
                    print(f'1. Transfer within GTCO\n2. Transfer to Other Banks.')
                    choice = int(input('Response: '))
                    ...
                    if choice == 1:
                        ...
                        # self.pin = int(input('Pin: '))
                        # record1 = "SELECT * FROM gtbank WHERE Pin=%s"
                        # val =(self.pin)
        ...
        # Airtime Purchase
            ...
            #    schedule = move.fetchone()
               ...
               if card_amount > self.balance:
                ...
                record = "SELECT * FROM gtbank WHERE Phone=%s"
                val =(phone_number)
                move.execute(record, val)
                schedule = move.fetchone()
                # print(schedule)
                gtcoairtimebal = float(schedule[6])
                #    print(f'Name: {schedule[1]} {schedule[2]} {schedule[-2]},  GTCO')
                if self.pin == schedule[-3]:
                    self.balance -= card_amount
                    self.airtime = gtcoairtimebal + card_amount
                    record = "UPDATE gtbank SET Balance=%s WHERE Pin=%s"
                    record1 = "UPDATE gtbank SET Airtime=%s WHERE Phone=%s"
                    val =(self.balance, self.pin)
            ...
            # Data Purchase
                ...
                if data_amount > self.balance:
                    ...
                    move.execute(record, val)
                    schedule = move.fetchone()
                    gtcodatabal = float(schedule[7])
                  
                    # print(schedule)                        
                    # #    print(f'Name: {schedule[1]} {schedule[2]} {schedule[-2]},  GTCO')
                    if self.pin == self.schedule[-3]:
                        self.balance -= data_amount
                        self.data = gtcodatabal + data_amount
                        
                        record = "UPDATE gtbank SET Balance=%s WHERE Pin=%s"
                        record1 = "UPDATE gtbank SET Data=%s WHERE Phone=%s"
